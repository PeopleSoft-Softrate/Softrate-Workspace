...
export class PipelineSectionComponent extends AdminWorkspaceSectionProxy implements OnInit, OnDestroy, DoCheck {
  ...
  ngDoCheck(): void {
    if (this.vm?.dashTab === 'pipeline') {
      const currentCompanyCode = (this.vm as any).companyCode || (this.vm as any).dashboardCode || '';
      if (currentCompanyCode && currentCompanyCode !== this.lastCompanyCode) {
        this.lastCompanyCode = currentCompanyCode;
        this.pipelineVm.init(currentCompanyCode);
      }
    }
  }
  ...
  viewCompanyProfile(deal: PipelineDeal): void {
    ...
    let fullLead: any = vm.allLeads?.find((l: any) => l._id === deal.leadId);
    if (!fullLead) {
      fullLead = {
        _id: deal.leadId,

...
class EmployeeLeaveApproval extends StatefulWidget {
  const EmployeeLeaveApproval({super.key});

  @override
  State<EmployeeLeaveApproval> createState() => _EmployeeLeaveApprovalState();
}

class _EmployeeLeaveApprovalState extends State<EmployeeLeaveApproval> {
  ...
  Future<List<LeaveRecord>> fetchPendingLeaves() async {
    final url = Uri.parse("${getBaseUrl()}/api/employee-leave/pending");
  ...
  Future<void> _updateLeaveStatus(LeaveRecord leave, String action) async {
    ...
    final url = Uri.parse("${getBaseUrl()}/api/employee-leave/${leave.id}");
    ...
    if (response.statusCode == 200) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Leave ${action == 'accepted' ? 'accepted' : 'rejected'}"),
        ...
        flexibleSpace: Container(
          decoration: const BoxDecoration(
            gradient: LinearGradient(
              colors: [Color(0xFF00657F), Color(0xFF004D60)],
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  ElevatedButton.icon(
                    onPressed: () => setState(() => _futureLeaves = fetchPendingLeaves()),
            ...
            return const Center(
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 8),
                  Text(
                    "Employees haven't requested any leaves yet",
                    style: TextStyle(
                      fontSize: 14,
                          ...
                          children: [
                            Expanded(
                              child: ElevatedButton.icon(
                                onPressed: () => _updateLeaveStatus(leave, 'accepted'),
                            ...
                            Expanded(
                              child: OutlinedButton.icon(
                                ...
                                style: OutlinedButton.styleFrom(
                                  side: const BorderSide(color: Color(0xFFB00020), width: 2),
                                  padding: const EdgeInsets.symmetric(vertical: 14),
                                  shape: RoundedRectangleBorder(
                                    borderRadius: BorderRadius.circular(30),
                                  ),

...
export class ActivitiesCalendarComponent implements OnInit, OnChanges {
  ...
  async generateCalendarDays() {
    if (this.employee?._id) {
      try {
        ...
        let url = `http://localhost:4000/api/activities/employee/${this.employee._id}?start=${firstDayOfMonth.toISOString()}&end=${lastDayOfMonth.toISOString()}`;
  ...
  async markActivityComplete(activity: any, event: Event) {
    ...
    try {
      const response = await fetch(`http://localhost:4000/api/activities/${activity._id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },

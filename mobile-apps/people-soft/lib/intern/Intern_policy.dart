...
class InternPolicyPage extends StatefulWidget {
  const InternPolicyPage({super.key});

  @override
  State<InternPolicyPage> createState() => _InternPolicyPageState();
}

class _InternPolicyPageState extends State<InternPolicyPage> {
  ...
  Future<void> _loadInternPolicies() async {
    setState(() => _loading = true);
    try {
      final res = await http.get(Uri.parse("${getBaseUrl()}/api/policy/all"));
      final List data = jsonDecode(res.body);

      // Filter only policies where "intern" is allowed
      _policies = data
          .where((p) => (p["policy_view_by"] as List).contains("intern"))
          .toList()
          .cast<Map<String, dynamic>>();
    } catch (e) {
      debugPrint("Error loading intern policies: $e");
      if (mounted) {
  ...
  void _viewPolicy(String url) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => PolicyPdfViewer(pdfUrl: url)),
    );
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              "No intern policies have been assigned yet.\nContact your HR department.",
  ...
  Widget _saveButton() {
    return Container(
      ...
      child: ElevatedButton.icon(
        onPressed: _loading ? null : _loadInternPolicies,

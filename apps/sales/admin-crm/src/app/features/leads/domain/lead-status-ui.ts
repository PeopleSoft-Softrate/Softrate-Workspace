...
export function leadStatusClass(status: string): string {
  ...
  if (normalized.includes('not') || normalized.includes('busy') || normalized.includes('invalid') || normalized.includes('dnp')) return 'status-not-interested';
...
export function leadStatusShortLabel(status: string): string {
  ...
  if (lower.includes('not') || lower.includes('dnp') || lower.includes('busy') || lower.includes('invalid')) return 'Not Connected';
...
export function leadStatusColor(status: string): string {
  ...
  if (normalized.includes('not') || normalized.includes('busy') || normalized.includes('invalid') || normalized.includes('dnp')) return 'var(--status-negative, #dc2626)';

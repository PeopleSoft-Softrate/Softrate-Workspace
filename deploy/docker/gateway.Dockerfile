...
WORKDIR /workspace/apps/hrms/emp-hr
COPY apps/hrms/emp-hr/package*.json ./
RUN npm ci --legacy-peer-deps --no-audit --no-fund || npm install --legacy-peer-deps --no-audit --no-fund
COPY apps/hrms/emp-hr/ ./
...
COPY --from=hrms-build /workspace/apps/hrms/emp-hr/dist/admin-page/browser/ /usr/share/nginx/html/hrms/

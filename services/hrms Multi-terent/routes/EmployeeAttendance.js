...
router.get("/export/pdf/employee/:employeeId", verifyTenant, async (req, res) => {
  try {
    ...
    try {
      ...
      console.log("Employee PDF Logo available:", !!logoData, logoData ? logoData.substring(0, 30) : "none");
    ...
    if (mongoose.Types.ObjectId.isValid(employeeId)) {
      queryConditions.push({ _id: employeeId });
    }
    const employeeInfo = await Employee.findOne({ $or: queryConditions });
    const fullName = employeeInfo ? (employeeInfo.fullName || employeeInfo.name || "Unknown") : "Unknown";

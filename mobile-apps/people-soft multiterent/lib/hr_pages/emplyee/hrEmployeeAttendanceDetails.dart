...
Future<List<LeaveRecord>> fetchLeavesForEmployee(String employeeId) async {
  final url = Uri.parse("${getBaseUrl()}/api/employee-leave/employee/$employeeId");
...
// ------------------- MAIN WIDGET -------------------
class HrEmployeeAttendanceDetails extends StatefulWidget {
  final String employeeId;
  final String employeeName;

  const HrEmployeeAttendanceDetails({
  ...
  @override
  State<HrEmployeeAttendanceDetails> createState() =>
      _HrEmployeeAttendanceDetailsState();
}

class _HrEmployeeAttendanceDetailsState extends State<HrEmployeeAttendanceDetails> {
  ...
  Future<void> _pickAndSaveTime(AttendanceRecord record, bool isPunchIn) async {
    ...
    if (existingTimeStr != null) {
      try {
        final dt = DateTime.parse(existingTimeStr);
    ...
    final TimeOfDay? picked = await showTimePicker(
      ...
      helpText: isPunchIn ? "SELECT PUNCH IN TIME" : "SELECT PUNCH OUT TIME",
    );
    ...
    if (picked != null) {
      ...
      final newISO = newDT.toIso8601String();

      if (isPunchIn) {
        await _updateAttendance(record.date, punchIn: newISO, punchOut: record.punchOutTime);
      } else {
        await _updateAttendance(record.date, punchIn: record.punchInTime, punchOut: newISO);
      }
    }
  }
      ...
      body: FutureBuilder<List<AttendanceRecord>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const Center(child: CircularProgressIndicator(color: Color(0xFF00657F)));
          }
            ...
            children: [
                ...
                child: Row(
                  children: [
                    _buildSummaryBox(
                      ...
                      label: "Present",
                      bgColor: const Color(0xFFE0F6EA),
              ...
              FutureBuilder<List<LeaveRecord>>(
                ...
                builder: (context, snap) {
                  ...
                  Color bg;
                  Color text;
                  if (nearestLeave.status.toLowerCase() == 'accepted') {
                    bg = const Color(0xFFE8F5E9);
                    text = const Color(0xFF2E7D32);
                  } else if (nearestLeave.status.toLowerCase() == 'rejected') {
  ...
  Future<void> _exportAttendancePdf({
    ...
    try {
      ...
      final fileName =
          "Attendance_${widget.employeeName}_${DateFormat('yyyyMMdd').format(fromDate)}.pdf";

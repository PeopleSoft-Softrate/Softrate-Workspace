...
class _AnalyticsTabState extends State<AnalyticsTab> {
  ...
  List<CallLogEntry> get _filteredLogs {
    ...
    final res = _allCallLogs.where((e) { final ts = e.timestamp ?? 0; return ts >= s && ts <= e2; }).toList();
    debugPrint('Analytics: ${res.length} logs matched period filter');
    return res;
  }
  ...
  Widget build(BuildContext context) {
    debugPrint('AnalyticsTab: Building UI (isLoading=$_isLoading, index=$_selectedFilter)');

...
class _PayrollPageState extends State<PayrollPage>
  ...
  Future<void> _loadPayrollData() async {
    ...
    try {
      ...
      final internId = prefs.getString('internMongoId') ?? '';
      if (internId.isEmpty) {
        throw Exception('Intern ID not found. Please log in again.');
      }
      ...
      final results = await Future.wait([
        http.get(Uri.parse('$base/api/intern/get/$internId')),
        http.get(Uri.parse('$base/api/settings/company')),
        http.get(Uri.parse('$base/api/holidays')),
        http.get(Uri.parse('$base/api/employee-leave/employee/$internId')),
        http.get(Uri.parse('$base/api/attendance/intern/$internId')),
      ]);

      // ── 1. Parse intern profile ───────────────────────────────────────────────
      final profileBody = jsonDecode(results[0].body);
      final profile = profileBody['intern'] ?? profileBody['data'] ?? profileBody;
      _internName = ('${profile['firstName'] ?? ''} ${profile['lastName'] ?? ''}').trim();
      if (_internName.isEmpty) _internName = (profile['fullName'] ?? '').toString().trim();
      _department = profile['department'] ?? profile['team'] ?? '';

      final payrollField = profile['payroll'] ?? profile['salary'] ?? profile['stipend'] ?? {};
      double b = _toDouble(payrollField['basicSalary'] ?? payrollField['basic']  ?? payrollField['stipendAmount']);
      _basic      = b > 0 ? b : 12000;
      double h = _toDouble(payrollField['hra']);
      _hra        = h > 0 ? h : 0;
      double a = _toDouble(payrollField['allowances']);
      _allowances = a > 0 ? a : 2000;
      double d = _toDouble(payrollField['deductions']);
      _deductions = d > 0 ? d : 1000;
      ...
      final lopEnabledIntern = lopSettings['enableLopIntern'];
      _enableLop       = lopEnabledIntern == true || lopEnabledIntern?.toString() == 'true';
      _lopType         = (lopSettings['lopTypeIntern'] ?? 'percentage').toString();
      _lopPercentage   = _toDouble(lopSettings['lopPercentageIntern'] ?? 100);
      _lopAmountPerDay = _toDouble(lopSettings['lopAmountIntern']     ?? 0);
      _workingDays     = _toInt(lopSettings['workingDaysIntern']      ?? 26);
  ...
  // ─── Helpers ─────────────────────────────────────────────────────────────────
  // Safe list extractor — handles bare JSON arrays and wrapped objects
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Payroll & Stipend',

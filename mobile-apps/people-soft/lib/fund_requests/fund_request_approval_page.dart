...
class FundRequestApprovalPage extends StatefulWidget {
  ...
  @override
  State<FundRequestApprovalPage> createState() => _FundRequestApprovalPageState();
}
...
class _FundRequestApprovalPageState extends State<FundRequestApprovalPage> {
  ...
  List<dynamic> _filtered(bool pendingOnly) {
    return _requests.where((request) {
      final status = _isManager ? request['managerStatus'] : request['hrStatus'];
      if (pendingOnly) {
  ...
  Future<String?> _askRemarks(String action) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text(isReject ? 'Reject fund request' : 'Approve fund request'),
        content: TextField(
  ...
  Future<void> _review(dynamic request, String status) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(success ? 'Fund request ${status == 'accepted' ? 'approved' : 'rejected'}' : 'Unable to update request'),
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Unable to update request: $e'), backgroundColor: Colors.red.shade700),
      );
    }
  }
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child: Center(
                  child: Text(
                    (request['requesterName'] ?? 'U').toString().substring(0, 1).toUpperCase(),
                    style: const TextStyle(color: primary, fontWeight: FontWeight.w900, fontSize: 18),
                  ),
                ),
              ),
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(request['requesterName'] ?? 'Unknown', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 15)),
                    Text(
              ...
              Text(_money(request['amount']), style: const TextStyle(color: primary, fontWeight: FontWeight.w900, fontSize: 16)),
            ],
          ),
          const SizedBox(height: 16),
          Text(request['category'] ?? 'Fund Request', style: const TextStyle(fontWeight: FontWeight.w800)),
          const SizedBox(height: 4),
          Text('Spent on ${_date(request['expenseDate'])}', style: const TextStyle(color: muted, fontSize: 12)),
          const SizedBox(height: 10),
          Text(request['description'] ?? '', style: const TextStyle(color: Color(0xFF334155), height: 1.45)),
          ...
          Wrap(
            ...
            children: [
              ...
              _chip('Finance', request['isFinanceTeamApprove'] == true ? 'accepted' : 'pending'),
            ],
          ),
          if (isPending) ...[
  ...
  Widget build(BuildContext context) {
    ...
    final subtitle = _isManager ? 'Manager review before HR' : 'Manager-approved reimbursement requests';
